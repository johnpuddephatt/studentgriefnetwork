<!-- Section 1 -->
<section class="relative pt-12 lg:pt-0">
  <img
    class="parallax-single-birds absolute z-40 h-auto w-40 lg:w-52 top-0 left-0 lg:top-auto lg:bottom-0 lg:left-[37.5%]"
    src="@asset('/images/home-video-single-bird.png')" />
  <img class="absolute z-40 hidden h-auto lg:block -right-16 top-32 parallax-two-birds w-72"
    src="@asset('/images/home-video-two-birds.png')" />
  <div class="container flex flex-col items-center gap-8 py-12 mx-auto lg:flex-row md:py-24">

    <div class="relative w-full lg:w-1/2">
      <a target="_blank" href="{{ $video->link ?? '#' }}"
        class="block relative rounded-lg overflow-hidden bg-blue-lightest pt-[56.25%]">
        <img class="absolute inset-0" src="@asset('/images/video-still.jpg')" />
        <svg class="absolute w-16 h-16 transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2"
          xmlns="http://www.w3.org/2000/svg" viewBox="0 0 146.84 163.27">
          <path fill="none" stroke="#fff" stroke-width="4"
            d="M1615.22,2512.11a18.49,18.49,0,0,1,0,32l-52.94,30.56-52.93,30.56a18.49,18.49,0,0,1-27.73-16V2467a18.49,18.49,0,0,1,27.73-16l52.93,30.56Z"
            transform="translate(-1479.62 -2446.48)" />
        </svg>

      </a>

    </div>
    <div class="lg:w-1/2 lg:pr-24">
      <h2 class="mb-4 text-2xl font-bold leading-tight lg:tracking-wide xl:text-3xl text-blue-dark font-kayaksans">{!!
        $video->title ?? 'title' !!}</h2>
      <p class="text-sm lg:text-base">{!! $video->description ?? 'desc' !!}</p>
      <a href="/about"
        class="inline-block px-4 py-3 mt-6 text-base font-semibold bg-white border-2 rounded-md border-blue-lighter text-blue-lighter">more
        about us</a>
    </div>


  </div>
</section>