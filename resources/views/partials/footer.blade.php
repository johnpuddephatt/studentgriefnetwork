<footer class="relative text-center lg:text-left">
  ...
  <div class="container relative z-10 hidden transform lg:block translate-y-52 ">
  </div>
  <div class="relative bg-opacity-50 lg:pt-24 text-blue footer-clip bg-blue-lighter lg:bg-white lg:bg-opacity-50 ">
    <div class="container py-16 pt-48 mx-auto lg:pt-32 max-w-none">
      <div class="flex flex-col gap-16 lg:gap-8 lg:flex-row">
        <div
          class="flex-none flex-grow-0 flex-shrink w-full pt-16 pl-6 mr-auto text-3xl font-bold text-center lg:-mb-16 font-kayaksans footer-logo md:w-auto">
          <div class="inline-block w-48 pb-32 pl-4 text-left stroke">
            {{ $siteName }}
          </div>
        </div>
        <div class="flex-shrink w-full mt-8 ml-auto lg:mt-6 lg:w-1/3 lg:mt-auto">
          <p class="mb-3 text-2xl font-semibold sm:mt-3 text-blue-light">{!! str_replace("|", "", $siteStrapline) !!}
          </p>

          @if(get_theme_mod('company_info'))
          <p class="mb-12 text-sm lg:mb-6 company-info">{{ get_theme_mod('company_info') }}</p>
          @endif
      ...
      <div class="flex-shrink w-full lg:mt-6 md:-ml-6 lg:ml-0 lg:mt-auto xl:w-auto lg:w-1/5 md:w-1/2">
        {!! $footerNavigation !!}
      </div>
      @endif

      @if(!empty($footerNavigation2))
      <div class="flex-shrink w-full lg:mt-6 md:-ml-6 lg:ml-0 lg:mt-auto xl:w-auto lg:w-1/5 md:w-1/2">
        {!! $footerNavigation2 !!}
      </div>
      @endif
